...
function Get-CSharpModelName
{
    ...
    [System.Action[string]]$script = {
        param(
            [string]
            $AssemblyName
        )

        [string]$AutoRestToolsPath = Join-Path -Path $env:USERPROFILE -ChildPath ".autorest\plugins\autorest\1.1.0"

        # if default directory does not exist try to fallback to old version of AutoRest
        if( -not (Test-Path $AutoRestToolsPath) ) {
            $AutoRestToolsPath = Get-Command -Name 'autorest' | 
        ...
        try {
            ...
            throw "Could not load file or assembly: $AssemblyFilePath -> $_"
        }
    }

    if (-not $script:CSharpCodeNamerLoadAttempted) {
        $script:CSharpCodeNamerLoadAttempted = $true
        $script:CSharpCodeNamer = New-ObjectFromDependency -TypeNames @('AutoRest.CSharp.CodeNamerCs', 'AutoRest.CSharp.CSharpCodeNamer') -AssemblyName 'AutoRest.CSharp.dll' -AssemblyResolver $script
    }
