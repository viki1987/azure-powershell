...
function Get-Table
{
    [OutputType([Microsoft.AzureStack.Storage.Admin.Models.TableServiceResponseResource])]
    [CmdletBinding(DefaultParameterSetName='Tables_Get')]
    param(    
        [Parameter(Mandatory = $true, ParameterSetName = 'Tables_Get')]
        [System.String]
        $ResourceGroupName,
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('Tables_Get' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation GetWithHttpMessagesAsync on $StorageAdminClient.'
        $taskResult = $StorageAdminClient.Tables.GetWithHttpMessagesAsync($ResourceGroupName, $FarmId)
    } else {
