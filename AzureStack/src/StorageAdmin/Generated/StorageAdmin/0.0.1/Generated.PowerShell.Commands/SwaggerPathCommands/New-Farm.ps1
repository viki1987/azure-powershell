...
function New-Farm
{
    ...
    param(    
        ...
        [Microsoft.AzureStack.Storage.Admin.Models.FarmCreateParameters]
        $Parameters,
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('Farms_Create' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation CreateWithHttpMessagesAsync on $StorageAdminClient.'
        $taskResult = $StorageAdminClient.Farms.CreateWithHttpMessagesAsync($ResourceGroupName, $FarmId, $Parameters)
    } else {
