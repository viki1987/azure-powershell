...
#>
function Stop-ContainerMigration
{
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('Containers_CancelMigration' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation CancelMigrationWithHttpMessagesAsync on $StorageAdminClient.'
        $taskResult = $StorageAdminClient.Containers.CancelMigrationWithHttpMessagesAsync($ResourceGroupName, $FarmId)
    } else {
