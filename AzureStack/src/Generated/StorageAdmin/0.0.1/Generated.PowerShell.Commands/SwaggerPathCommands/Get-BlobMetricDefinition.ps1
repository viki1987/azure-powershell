...
#>
function Get-BlobMetricDefinition
{
    [CmdletBinding(DefaultParameterSetName='Blobs_ListMetricDefinitions')]
    param(    
        [Parameter(Mandatory = $true, ParameterSetName = 'Blobs_ListMetricDefinitions')]
        [System.String]
        $ResourceGroupName,
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('Blobs_ListMetricDefinitions' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation ListMetricDefinitionsWithHttpMessagesAsync on $StorageAdminClient.'
        $taskResult = $StorageAdminClient.Blobs.ListMetricDefinitionsWithHttpMessagesAsync($ResourceGroupName, $FarmId)
    } else {
