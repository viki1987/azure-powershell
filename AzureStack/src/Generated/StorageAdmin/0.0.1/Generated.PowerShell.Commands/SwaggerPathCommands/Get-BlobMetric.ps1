...
#>
function Get-BlobMetric
{
    [OutputType([Microsoft.AzureStack.Storage.Admin.Models.Metric])]
    [CmdletBinding(DefaultParameterSetName='Blobs_ListMetrics')]
    param(    
        [Parameter(Mandatory = $true, ParameterSetName = 'Blobs_ListMetrics')]
        [System.String]
        $Filter,
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('Blobs_ListMetrics' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation ListMetricsWithHttpMessagesAsync on $StorageAdminClient.'
        $taskResult = $StorageAdminClient.Blobs.ListMetricsWithHttpMessagesAsync($ResourceGroupName, $FarmId, $Filter)
    } else {
