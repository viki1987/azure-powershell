...
#>
function Get-FarmGarbageCollectState
{
    [CmdletBinding(DefaultParameterSetName='Farms_GetGarbageCollectState')]
    param(    
        [Parameter(Mandatory = $true, ParameterSetName = 'Farms_GetGarbageCollectState')]
        [System.String]
        $OperationId,
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('Farms_GetGarbageCollectState' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation GetGarbageCollectStateWithHttpMessagesAsync on $StorageAdminClient.'
        $taskResult = $StorageAdminClient.Farms.GetGarbageCollectStateWithHttpMessagesAsync($ResourceGroupName, $FarmId, $OperationId)
    } else {
