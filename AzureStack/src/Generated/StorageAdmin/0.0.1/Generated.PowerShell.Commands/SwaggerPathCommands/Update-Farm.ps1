...
function Update-Farm
{
    ...
    param(    
        ...
        [Microsoft.AzureStack.Storage.Admin.Models.FarmUpdateParameters]
        $Parameters,
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('Farms_Update' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation UpdateWithHttpMessagesAsync on $StorageAdminClient.'
        $taskResult = $StorageAdminClient.Farms.UpdateWithHttpMessagesAsync($ResourceGroupName, $FarmId, $Parameters)
    } else {
