...
#>
function Sync-StorageAccountAll
{
    [OutputType([Microsoft.AzureStack.Storage.Admin.Models.StorageAccountSyncResult])]
    [CmdletBinding(DefaultParameterSetName='StorageAccounts_SynchronizeAll')]
    param(    
        [Parameter(Mandatory = $true, ParameterSetName = 'StorageAccounts_SynchronizeAll')]
        [System.String]
        $TenantId,
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('StorageAccounts_SynchronizeAll' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation SynchronizeAllWithHttpMessagesAsync on $StorageAdminClient.'
        $taskResult = $StorageAdminClient.StorageAccounts.SynchronizeAllWithHttpMessagesAsync($TenantId, $FarmId, $AccountId)
    } else {
