...
{
    [OutputType([Microsoft.AzureStack.Storage.Admin.Models.StorageAccountModel])]
    [CmdletBinding(DefaultParameterSetName='StorageAccounts_List')]
    param(    
    ...
    if ('StorageAccounts_List' -eq $PsCmdlet.ParameterSetName) {
        ...
        $taskResult = $StorageAdminClient.StorageAccounts.ListWithHttpMessagesAsync($ResourceGroupName, $FarmId, $Filter, $Summary)
    } elseif ('StorageAccounts_Get' -eq $PsCmdlet.ParameterSetName ) {
