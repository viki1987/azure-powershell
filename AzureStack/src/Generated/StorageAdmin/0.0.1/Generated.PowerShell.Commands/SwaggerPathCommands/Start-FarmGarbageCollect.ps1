...
#>
function Start-FarmGarbageCollect
{
    [CmdletBinding(DefaultParameterSetName='Farms_StartGarbageCollect')]
    param(    
        [Parameter(Mandatory = $true, ParameterSetName = 'Farms_StartGarbageCollect')]
        [System.String]
        $ResourceGroupName,
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('Farms_StartGarbageCollect' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation StartGarbageCollectWithHttpMessagesAsync on $StorageAdminClient.'
        $taskResult = $StorageAdminClient.Farms.StartGarbageCollectWithHttpMessagesAsync($ResourceGroupName, $FarmId)
    } else {
