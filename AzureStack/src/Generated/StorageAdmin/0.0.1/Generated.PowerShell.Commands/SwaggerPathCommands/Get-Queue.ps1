...
function Get-Queue
{
    [OutputType([Microsoft.AzureStack.Storage.Admin.Models.QueueServiceResponseResource])]
    [CmdletBinding(DefaultParameterSetName='Queues_Get')]
    param(    
        [Parameter(Mandatory = $true, ParameterSetName = 'Queues_Get')]
        [System.String]
        $ResourceGroupName,
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('Queues_Get' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation GetWithHttpMessagesAsync on $StorageAdminClient.'
        $taskResult = $StorageAdminClient.Queues.GetWithHttpMessagesAsync($ResourceGroupName, $FarmId)
    } else {
