...
#>
function Get-TableMetricDefinition
{
    [CmdletBinding(DefaultParameterSetName='Tables_ListMetricDefinitions')]
    param(    
        [Parameter(Mandatory = $true, ParameterSetName = 'Tables_ListMetricDefinitions')]
        [System.String]
        $ResourceGroupName,
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('Tables_ListMetricDefinitions' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation ListMetricDefinitionsWithHttpMessagesAsync on $StorageAdminClient.'
        $taskResult = $StorageAdminClient.Tables.ListMetricDefinitionsWithHttpMessagesAsync($ResourceGroupName, $FarmId)
    } else {
