...
#>
function Get-QueueMetricDefinition
{
    [CmdletBinding(DefaultParameterSetName='Queues_ListMetricDefinitions')]
    param(    
        [Parameter(Mandatory = $true, ParameterSetName = 'Queues_ListMetricDefinitions')]
        [System.String]
        $Filter,
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('Queues_ListMetricDefinitions' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation ListMetricDefinitionsWithHttpMessagesAsync on $StorageAdminClient.'
        $taskResult = $StorageAdminClient.Queues.ListMetricDefinitionsWithHttpMessagesAsync($ResourceGroupName, $FarmId, $Filter)
    } else {
